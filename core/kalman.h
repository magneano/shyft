            ...
            */
            ...
            struct filter {
                ...
                inline int fold_to_daily_observation(utctime t) const {
                    return ((to_seconds64(t)/to_seconds64(deltahours(1)) + 200L*24*365) % 24)*p.n_daily_observations/24;// in short, add 200 years (ensure positve num before %)
                }
            ...
            struct bias_predictor {
                ...
                a_ts compute_running_bias(const fc_ts& fcts,const obs_ts& observation_ts, const ta& time_axis) {
                    ...
                    auto pd_dt = shyft::core::deltahours(24)/f.p.n_daily_observations;
