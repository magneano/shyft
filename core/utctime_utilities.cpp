...
string calendar::to_string(utctime t) const {
    ...
    if (t == no_utctime) { sprintf(s, "no_utctime"); }
        ...
        if(us==0)
            ...
            sprintf(s, "%04d-%02d-%02dT%02d:%02d:%02d.%06d%s", c.year, c.month, c.day, c.hour, c.minute, c.second,int(us),tzs);
    }
    return string(s);
}
...
utctime calendar::time(YMDhms c) const {
    ...
    utctime r= utctime(std::chrono::seconds( (day_number(c) - UnixDay)*86400LL) + hms_seconds(c.hour, c.minute, c.second,c.micro_second));
...
utctime calendar::time_from_week(int Y, int W, int wd, int h, int m, int s,int us) const {
    return time(YWdhms(Y, W, wd, h, m, s,us));
}
utctime calendar::time(YWdhms c) const {
    ...
    utctime r = utctime(chrono::seconds( (int(w_daynumber) - UnixDay)*86400LL) + hms_seconds(c.hour, c.minute, c.second,c.micro_second));// make it local utc-time
...
static inline C& fill_in_hms_from_t(utctime& t, C& r) {
    long long ts=chrono::duration_cast<chrono::seconds>(t).count();
    long long us= (t-seconds(ts)).count();
    if(us<0) { //borrow from seconds to always have positive usec
        ts-=1;
        t-=seconds(1);
        us+=utctime(seconds(1)).count();
    }
    long long tj = calendar::UnixSecond + ts;
...
YWdhms calendar::calendar_week_units(utctime t) const {
    ...
    t += tz_dt;
    YWdhms r;
    fill_in_hms_from_t(t,r);// might modify t, if before 1970 and us
    auto jdn = day_number(t);
...
namespace time_zone {
    ...
    struct boost_tz_info { // limitation is posix time, def from 1901 and onward
        ...
        utctimespan base_offset() const {
            return seconds(tzinfo->base_utc_offset().total_seconds());
        }
        utctimespan dst_offset(int y) const {
            return seconds(tzinfo->dst_offset().total_seconds());
        }
...
utcperiod utcperiod::trim(const calendar &c, utctimespan deltaT, trim_policy tp=trim_policy::TRIM_IN) const {
    utcperiod trimmed_utcperiod{start,end};
    if(!trimmed_utcperiod.valid()) {
        throw runtime_error("attempt to trim invalid/null period");
    }
    if(deltaT<= utctimespan{0}) {
        throw runtime_error("attempt to trim with invalid delta-time");
    }
    if (tp==trim_policy::TRIM_IN) {
        trimmed_utcperiod.start = c.trim(c.add(start, deltaT, 1) - seconds{1}, deltaT);
        trimmed_utcperiod.end = c.trim(end, deltaT);
    }
    else {
        trimmed_utcperiod.start = c.trim(start, deltaT);
        trimmed_utcperiod.end = c.trim(c.add(end, deltaT, 1) - seconds{1}, deltaT);
    }
    return trimmed_utcperiod;
}

int64_t utcperiod::diff_units(const calendar &c, utctimespan deltaT) const {
    return c.diff_units(start, end, deltaT);
}
int64_t utcperiod::diff_units(const calendar &c, utctimespan deltaT, utctimespan &remainder) const {
