...
void test_linear_fx(F&& acc_fn) {
	...
	TEST_SECTION("out_of_points_searching_for_start") {
        point_ts<fixed_dt> nts(fixed_dt{_t(0),dt,5},vector<double>{shyft::nan,1.0,1.0,shyft::nan,shyft::nan},linear);
	...
	TEST_SECTION("just_nans") {
        point_ts<fixed_dt> nts(fixed_dt{_t(0),dt,5},vector<double>{shyft::nan,shyft::nan,shyft::nan,shyft::nan,shyft::nan},linear);
        fixed_dt ta2{_t(0),dt*4,2};
        auto r = acc_fn(ta2,nts,true);
        FAST_REQUIRE_EQ(r.size(),ta2.size());
...
void test_stair_case_fx(F&& acc_fn) {
	...
	TEST_SECTION("out_of_points_searching_for_start") {
        point_ts<fixed_dt> nts(fixed_dt{_t(0),dt,5},vector<double>{shyft::nan,1.0,1.0,shyft::nan,shyft::nan},stair_case);
        fixed_dt ta2{_t(0),dt*4,2};
        auto r = acc_fn(ta2,nts,true);
        FAST_REQUIRE_EQ(r.size(),ta2.size());
        CHECK(r[0]==doctest::Approx(1.0));
	...
	TEST_SECTION("just_nans") {
        point_ts<fixed_dt> nts(fixed_dt{_t(0),dt,5},vector<double>{shyft::nan,shyft::nan,shyft::nan,shyft::nan,shyft::nan},stair_case);
        fixed_dt ta2{_t(0),dt*4,2};
        auto r = acc_fn(ta2,nts,true);
        FAST_REQUIRE_EQ(r.size(),ta2.size());
        FAST_CHECK_UNARY(!isfinite(r[0]));
...
TEST_SUITE("time_series") {
    ...
    TEST_CASE("ts_avg_speed_test") {
        ...
        std::cout << "Stair  mill pts/sec " << mpts_s / (us_l / 1e6) << "\n";
        std::cout << "Linear mill pts/sec " << mpts_s / (us_s / 1e6) << "\n";
