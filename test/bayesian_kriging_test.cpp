...
std::vector<utctime> ctimes{ _t(0), _t(3600) };
...
void build_sources_and_dests(const size_t num_sources_x, const size_t num_sources_y,
    ...
    for (size_t l = 0; l < ts_size; ++l)
        times.emplace_back(utctime(l*dt));
...
TEST_CASE("test_build_covariance_matrices") {
	...
	build_sources_and_dests(3, 3, 15, 15, 2, seconds(10), time_axis, false, sources, destinations);
	arma::mat K, k;
