...
TEST_CASE("time_axis_merge_fixed_dt") {
    ...
    CHECK_THROWS(merge(fixed_dt{ t0,dt,4 }, fixed_dt{ t2,dt2,4 }));
...
TEST_CASE("time_axis_f_slice") {
    ...
    auto b=a.slice(1,2);
    FAST_CHECK_EQ(b.time(0),utctime{seconds(1)});
    FAST_CHECK_EQ(b.time(1),utctime{seconds(2)});
...
TEST_CASE("time_axis_p_slice") {
    vector<utctime> tp;for(int64_t t=0;t<10;++t) tp.push_back(_t(t));
    ...
    FAST_CHECK_EQ(c.time(0),utctime{seconds(8)});
    FAST_CHECK_EQ(c.time(1),utctime{seconds(9)});
}
TEST_CASE("time_axis_g_slice") {
    /*fixed */ {
        time_axis::generic_dt a(time_axis::fixed_dt(_t(0),seconds(1),10));
        auto b=a.slice(1,2);
        FAST_CHECK_EQ(b.time(0),_t(1));
        FAST_CHECK_EQ(b.time(1),_t(2));
        FAST_CHECK_EQ(b.size(),2);
    }
    /* calendar*/ {
    ...
    /* point */{
    time_axis::generic_dt a(time_axis::point_dt(vector<utctime>{_t(0),_t(1),_t(2),_t(3),_t(4),_t(5),_t(6),_t(7),_t(8),_t(9)},_t(10)));
    auto b=a.slice(1,2);
    FAST_CHECK_EQ(b.time(0),_t(1));
    FAST_CHECK_EQ(b.time(1),_t(2));
    FAST_CHECK_EQ(b.size(),2);
    auto c=a.slice(8,2);
    FAST_CHECK_EQ(c.size(),2);
    FAST_CHECK_EQ(c.time(0),_t(8));
    FAST_CHECK_EQ(c.time(1),_t(9));
    }
